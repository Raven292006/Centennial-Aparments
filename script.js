...
function bindGlobalUi() {
  ...
  if (loginTabBtn) loginTabBtn.addEventListener('click', () => openAuthModal('inquiry'));

  const tenantTabBtn = document.getElementById('tenant-tab-btn');
  if (tenantTabBtn) tenantTabBtn.addEventListener('click', () => openAuthModal('tenant'));

  const adminTabBtn = document.getElementById('admin-tab-btn');
  if (adminTabBtn) adminTabBtn.addEventListener('click', () => openAuthModal('admin'));

  const registerTabBtn = document.getElementById('register-tab-btn');
  if (registerTabBtn) registerTabBtn.addEventListener('click', () => openAuthModal('register'));
...
function syncAuthUi() {
  ...
  if (currentUser && currentUser.role === 'inquiry') {
    ...
    if (inquiryLogout) inquiryLogout.style.display = 'inline';
    return;
  }
...
function openAuthModal(type) {
  ...
  const activeTab = document.getElementById(tabs[type] || tabs.inquiry);
...
function handleInquiryLogin(event) {
  ...
  if (registeredUser) {
    ...
    closeAuthModal();
    syncAuthUi();
    if (localStorage.getItem(STORAGE_KEYS.selectedBookingUnit) && !window.location.pathname.endsWith('booking.html')) {
      window.location.href = 'booking.html';
      return;
    }

    alert('Logged in successfully. You can now submit inquiries and bookings.');
    return;
  }
...
function handleTenantLogin(event) {
  ...
  alert('Invalid tenant credentials. Use tenant@example.com / password.');
}
...
function handleAdminLogin(event) {
  ...
  alert('Invalid admin credentials. Use admin@example.com / SecureAdmin@2026.');
}
...
function initializeTenantPortal() {
  ...
  setText('dashboard-unit', tenant ? tenant.unit : currentUser.unit || 'Unassigned');
  setText('dashboard-floor', unit ? unit.floor : currentUser.floor || 'Unassigned');
...
function loadTenantAnnouncements() {
  ...
  setText('announcement-count', `${publishedAnnouncements.length} active announcements`);
}
  ...
  tbody.innerHTML = tenantBills
    .map(item => `
      <tr>
        ...
        <td>${item.status === 'paid' ? '<span>Paid</span>' : `<button class="btn-small" onclick="payBill('${item.id}')">Pay Now</button>`}</td>
      </tr>
    `)
    .join('');
...
function payBill(id) {
  ...
  refreshState();
  loadTenantBills();
  alert(`Payment recorded for ${bill.month}.`);
}
...
function loadAdminBills() {
      ...
      return tenant
        ? `
          <tr>
            ...
            <td><span class="status-badge ${statusClass(item.status)}">${item.status}</span></td>
            <td>${item.status === 'paid' ? '<span>Paid</span>' : `<button class="btn-small" onclick="markBillPaid('${item.id}')">Mark Paid</button>`}</td>
          </tr>
        `
        : '';
    })
  ...
  if (billingCards.length === 3) {
    const expected = bills.reduce((sum, item) => sum + item.total, 0);
